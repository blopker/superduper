...
class BikeConnectionManager {
  ...
  void _initialize() {
    ...
    _bikesSubscription = _ref.read(bikeRepositoryProvider).bikesStream.listen((bikes) {
  ...
  void _updateMonitoredBikes(List<BikeModel> bikes) {
    ...
    final currentActiveIds = bikes.where((b) => b.isActive).map((b) => b.id).toSet();
    ...
    for (final bikeId in inactiveIds) {
      final service = repository.getBikeService(bikeId);
      if (service != null) {
        log.d(SDLogger.BIKE, 'Stopping monitoring of bike: ${service.bike.name}');
        service.disconnect();
      }
